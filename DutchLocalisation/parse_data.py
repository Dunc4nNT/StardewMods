...
def parse() -> None:
    ...
    Path(f"./data/output-{folderdate}/assets/vanilla/").mkdir(
    ...
    for file in german_data:
        gamefilepath = (
            str(file.parent).replace("data\\input\\vanilla\\", "").replace("\\", "/")
        )
        ...
        _ = (
            Path(f"{outputpath_vanilla}/{gamefilepath}").mkdir(
                parents=True, exist_ok=True
            ),
        )
        shutil.copy(
            Path(str(file)),
            Path(f"{outputpath_vanilla}/{gamefilepath}/{file.name}"),
        )
